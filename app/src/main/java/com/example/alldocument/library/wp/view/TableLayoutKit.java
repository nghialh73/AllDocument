 ...
 * 时间:              上午11:26:53
 */
package com.example.alldocument.library.wp.view;
...
import com.example.alldocument.library.simpletext.view.DocAttr;
import com.example.alldocument.library.simpletext.view.IRoot;
...
import com.example.alldocument.library.simpletext.view.ParaAttr;
import com.example.alldocument.library.simpletext.view.TableAttr;
import com.example.alldocument.library.simpletext.view.ViewKit;
...
import com.example.alldocument.library.wp.view.BreakPagesCell;
import com.example.alldocument.library.wp.view.CellView;
import com.example.alldocument.library.wp.view.LayoutKit;
import com.example.alldocument.library.wp.view.ParagraphView;
import com.example.alldocument.library.wp.view.RowView;
import com.example.alldocument.library.wp.view.TableView;
import com.example.alldocument.library.wp.view.ViewFactory;
...
{   
    ...
    {
        breakPagesCell = new LinkedHashMap<Integer, com.example.alldocument.library.wp.view.BreakPagesCell>();
    }
    /**
     * 布局段浇
     * @param docAttr       文档属性
                           ...
                           TableView tableView, long startOffset, int x, int y, int w, int h, int flag, boolean isBreakPages)
    {
        ...
        int tableHeight = 0;
        int tableWidth = 0;
        com.example.alldocument.library.wp.view.RowView rowView = null;
            ...
            || (breakRowElement != null && isBreakPages))
        {
            ...
            rowView = (com.example.alldocument.library.wp.view.RowView) com.example.alldocument.library.wp.view.ViewFactory.createView(control, rowElem, null, WPViewConstant.TABLE_ROW_VIEW);
            ...
            {
                com.example.alldocument.library.wp.view.RowView preView = (com.example.alldocument.library.wp.view.RowView)rowView.getPreView();
    ...
    private void clearCurrentRowBreakPageCell(IElement currentElem)
    {
        ...
        {
            com.example.alldocument.library.wp.view.BreakPagesCell bc = breakPagesCell.get(key);
    ...
    public int layoutRow(IControl control, IDocument doc, IRoot root, DocAttr docAttr, PageAttr pageAttr, ParaAttr paraAttr,
                         com.example.alldocument.library.wp.view.RowView rowView, long startOffset, int x, int y, int w, int h, int flag, boolean isBreakPages)
    {
        int dx = 0;
        int dy = 0;
        int breakType =  WPViewConstant.BREAK_NO;
            ...
            /*|| breakPagesCell.size() > 0 && isBreakPages*/)
        {
            ...
            if (isBreakPages && breakPagesCell.size() > 0)
            {
                ...
                {
                    com.example.alldocument.library.wp.view.BreakPagesCell breakCell = breakPagesCell.remove(cellIndex);
            ...
            {   
                ...
                {
                    com.example.alldocument.library.wp.view.CellView temp = breakRowView.getCellView((short)cellIndex);
                    if (temp != null)
                    {
            ...
            com.example.alldocument.library.wp.view.CellView cellView = (com.example.alldocument.library.wp.view.CellView) com.example.alldocument.library.wp.view.ViewFactory.createView(control, cellElem, null, WPViewConstant.TABLE_CELL_VIEW);
        ...
        //
        com.example.alldocument.library.wp.view.CellView cellView = (com.example.alldocument.library.wp.view.CellView)rowView.getChildView();
        while (cellView != null)
        {
            if (!cellView.isMergedCell()
            ...
            cellView = (com.example.alldocument.library.wp.view.CellView)cellView.getNextView();
        }
    ...
    public int layoutCell(IControl control, IDocument doc, IRoot root, DocAttr docAttr, PageAttr pageAttr, ParaAttr paraAttr,
                          com.example.alldocument.library.wp.view.CellView cellView, long startOffset, int x, int y, int w, int h, int flag, int cellIndex, boolean isBreakPages)
    {        
        CellElement cellElem = (CellElement)cellView.getElement();
        ...
        while (startOffset < maxEnd && spanH > 0 && breakType != WPViewConstant.BREAK_LIMIT)
        {
            ...
            com.example.alldocument.library.wp.view.ParagraphView paraView = (ParagraphView) ViewFactory.createView(control, paraElem, null, WPViewConstant.PARAGRAPH_VIEW);
        ...
        if (startOffset < maxEnd)
        {
            ...
            {
                breakPagesCell.put(cellIndex, new com.example.alldocument.library.wp.view.BreakPagesCell(cellElem, startOffset));
     ...
     * 
     */
    private void layoutMergedCell(com.example.alldocument.library.wp.view.RowView row, RowElement nextRowElem, boolean isLastRow)
    {       
        if (row == null)
        {
            return;
        ...
        if (isLastRow)
        {
            for (com.example.alldocument.library.wp.view.CellView cell : mergedCell)
            {
        ...
        Vector<com.example.alldocument.library.wp.view.CellView> vector = new Vector<com.example.alldocument.library.wp.view.CellView>();
        for(com.example.alldocument.library.wp.view.CellView cell : mergedCell)
        {            
                ...
                || AttrManage.instance().isTableVerFirstMerged(cellElem.getAttribute()))
            {
                ...
                else
                {
                    ...
                    com.example.alldocument.library.wp.view.CellView cellView = (com.example.alldocument.library.wp.view.CellView)row.getChildView();
                    while (cellView != null)
                    {
                        if (!cellView.isMergedCell())
                        {
        ...
        //
        for (com.example.alldocument.library.wp.view.CellView cell : vector)
        {
     ...
     * 判断给行是否跨行
     */
    private boolean isBreakPages(com.example.alldocument.library.wp.view.RowView rowView)
    {
     ...
     * 
     */
    private void layoutCellVerticalAlign(com.example.alldocument.library.wp.view.CellView cellView)
    {
    ...
    //
    private Vector<com.example.alldocument.library.wp.view.CellView> mergedCell = new Vector<CellView>();
}
