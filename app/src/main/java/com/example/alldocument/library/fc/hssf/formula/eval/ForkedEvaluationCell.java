...
final class ForkedEvaluationCell implements EvaluationCell {
	...
	public ForkedEvaluationCell(ForkedEvaluationSheet sheet, EvaluationCell masterCell) {
		...
		setValue(com.example.alldocument.library.fc.hssf.formula.eval.BlankEval.instance); // followed by a proper call to setValue()
	}
	...
	public void setValue(com.example.alldocument.library.fc.hssf.formula.eval.ValueEval value) {
		Class<? extends ValueEval> cls = value.getClass();

		if (cls == com.example.alldocument.library.fc.hssf.formula.eval.NumberEval.class) {
			_cellType = ICell.CELL_TYPE_NUMERIC;
			_numberValue = ((NumberEval)value).getNumberValue();
			return;
		}
		if (cls == com.example.alldocument.library.fc.hssf.formula.eval.StringEval.class) {
			_cellType = ICell.CELL_TYPE_STRING;
			_stringValue = ((StringEval)value).getStringValue();
			return;
		}
		if (cls == com.example.alldocument.library.fc.hssf.formula.eval.BoolEval.class) {
			_cellType = ICell.CELL_TYPE_BOOLEAN;
			_booleanValue = ((BoolEval)value).getBooleanValue();
			return;
		}
		if (cls == com.example.alldocument.library.fc.hssf.formula.eval.ErrorEval.class) {
