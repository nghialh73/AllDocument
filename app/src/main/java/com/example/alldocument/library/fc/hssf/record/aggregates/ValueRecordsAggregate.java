	...
	public void visitCellsForRow(int rowIndex, RecordVisitor rv) {
		...
		for (int i = 0; i < rowCells.length; i++) {
			...
			if (nBlank > 1) {
				...
				i+=nBlank-1;
			} else if (cvr instanceof com.example.alldocument.library.fc.hssf.record.aggregates.RecordAggregate) {
				com.example.alldocument.library.fc.hssf.record.aggregates.RecordAggregate agg = (RecordAggregate) cvr;
