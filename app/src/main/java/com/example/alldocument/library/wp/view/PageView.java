...
public class PageView extends AbstractView
{
    ...
    private void drawShape(Canvas canvas, int originX, int originY, float zoom, boolean drawBehindDocShape)
    {
        ...
        {
            //behind doc
            for (com.example.alldocument.library.wp.view.LeafView shape : shapeViews)
            {
                if(shape instanceof ShapeView && ((ShapeView)shape).isBehindDoc())
                {
                	((ShapeView)shape).drawForWrap(canvas, originX, originY, zoom);                 
                }
                else if(shape instanceof com.example.alldocument.library.wp.view.ObjView && ((com.example.alldocument.library.wp.view.ObjView)shape).isBehindDoc())
                {
                    ((com.example.alldocument.library.wp.view.ObjView)shape).drawForWrap(canvas, originX, originY, zoom);
                }
            }
        }
        else
     ...
     * @param view
     */
    public void addShapeView(com.example.alldocument.library.wp.view.LeafView view)
    {
        if (shapeViews == null)
        {
            shapeViews = new ArrayList<com.example.alldocument.library.wp.view.LeafView>();
        }
        shapeViews.add(view);            
    }
    	...
    	if(titleView != null)
    	{
    		...
    		while (paraView != null)
            {
    			...
    			while(lineView != null)
    			{
    				...
    				while(leafView != null)
    				{
    					if(leafView instanceof com.example.alldocument.library.wp.view.LeafView)
    					{
    						if(((com.example.alldocument.library.wp.view.LeafView)leafView).hasUpdatedFieldText())
    						{
    							hasTotalPageCode = true;
    							((com.example.alldocument.library.wp.view.LeafView)leafView).setNumPages(totalPages);
    						}
    					}    					
