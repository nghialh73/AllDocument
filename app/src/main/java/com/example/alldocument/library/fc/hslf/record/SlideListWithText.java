...
public final class SlideListWithText extends RecordContainer
{
    ...
    protected SlideListWithText(byte[] source, int start, int len)
    {
        ...
        {
            if (_children[i] instanceof com.example.alldocument.library.fc.hslf.record.SlidePersistAtom)
            {
                // Find where the next SlidePersistAtom is
                int endPos = i + 1;
                while (endPos < _children.length
                    && !(_children[endPos] instanceof com.example.alldocument.library.fc.hslf.record.SlidePersistAtom))
                {
                    endPos += 1;
                }
                ...
                com.example.alldocument.library.fc.hslf.record.Record[] spaChildren = new com.example.alldocument.library.fc.hslf.record.Record[clen];
                System.arraycopy(_children, i + 1, spaChildren, 0, clen);
                SlideAtomsSet set = new SlideAtomsSet((com.example.alldocument.library.fc.hslf.record.SlidePersistAtom)_children[i], spaChildren);
                sets.add(set);
    ...
    public SlideListWithText()
    {
        ...
        _children = new com.example.alldocument.library.fc.hslf.record.Record[0];
     ...
     * @param spa
     */
    public void addSlidePersistAtom(com.example.alldocument.library.fc.hslf.record.SlidePersistAtom spa)
    {
        // Add the new SlidePersistAtom at the end
        appendChildRecord(spa);

        SlideAtomsSet newSAS = new SlideAtomsSet(spa, new com.example.alldocument.library.fc.hslf.record.Record[0]);
    ...
    */
    ...
    {
        private com.example.alldocument.library.fc.hslf.record.SlidePersistAtom slidePersistAtom;
        private com.example.alldocument.library.fc.hslf.record.Record[] slideRecords;

        /** Get the SlidePersistAtom, which gives details on the Slide this text is associated with */
        public com.example.alldocument.library.fc.hslf.record.SlidePersistAtom getSlidePersistAtom()
        {
            return slidePersistAtom;
        }

        /** Get the Text related records for this slide */
        public com.example.alldocument.library.fc.hslf.record.Record[] getSlideRecords()
        {
            return slideRecords;
        }

        /** Create one to hold the Records for one Slide's text */
        public SlideAtomsSet(SlidePersistAtom s, com.example.alldocument.library.fc.hslf.record.Record[] r)
        {
            slidePersistAtom = s;
            slideRecords = r;
        }
