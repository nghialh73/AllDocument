...
public abstract class TwoOperandNumericOperation extends Fixed2ArgFunction {
	...
	protected final double singleOperandEvaluate(com.example.alldocument.library.fc.hssf.formula.eval.ValueEval arg, int srcCellRow, int srcCellCol) throws com.example.alldocument.library.fc.hssf.formula.eval.EvaluationException {
		com.example.alldocument.library.fc.hssf.formula.eval.ValueEval ve = com.example.alldocument.library.fc.hssf.formula.eval.OperandResolver.getSingleValue(arg, srcCellRow, srcCellCol);
		return OperandResolver.coerceValueToDouble(ve);
	}
	public com.example.alldocument.library.fc.hssf.formula.eval.ValueEval evaluate(int srcRowIndex, int srcColumnIndex, com.example.alldocument.library.fc.hssf.formula.eval.ValueEval arg0, ValueEval arg1) {
		double result;
		try {
			...
			if (result == 0.0) { // this '==' matches +0.0 and -0.0
				...
				if (!(this instanceof SubtractEvalClass)) {
					return com.example.alldocument.library.fc.hssf.formula.eval.NumberEval.ZERO;
				}
			}
			if (Double.isNaN(result) || Double.isInfinite(result)) {
				return com.example.alldocument.library.fc.hssf.formula.eval.ErrorEval.NUM_ERROR;
			}
		} catch (com.example.alldocument.library.fc.hssf.formula.eval.EvaluationException e) {
			return e.getErrorEval();
		}
		return new NumberEval(result);
	}

	protected abstract double evaluate(double d0, double d1) throws com.example.alldocument.library.fc.hssf.formula.eval.EvaluationException;
	...
	public static final Function DivideEval = new TwoOperandNumericOperation() {
		protected double evaluate(double d0, double d1) throws com.example.alldocument.library.fc.hssf.formula.eval.EvaluationException {
			if (d1 == 0.0) {
