...
import com.example.alldocument.library.fc.hwpf.sprm.SprmUncompressor;
...
public final class TableSprmUncompressor extends SprmUncompressor
{
    ...
    public static TableProperties uncompressTAP(byte[] grpprl, int offset)
    {
        ...
        com.example.alldocument.library.fc.hwpf.sprm.SprmIterator sprmIt = new com.example.alldocument.library.fc.hwpf.sprm.SprmIterator(grpprl, offset);

        while (sprmIt.hasNext())
        {
            com.example.alldocument.library.fc.hwpf.sprm.SprmOperation sprm = sprmIt.next();

            //TAPXs are actually PAPXs so we have to make sure we are only trying to
            //uncompress the right type of sprm.
            if (sprm.getType() == com.example.alldocument.library.fc.hwpf.sprm.SprmOperation.TAP_TYPE)
            {
                try
                {
    ...
    public static TableProperties uncompressTAP(SprmBuffer sprmBuffer)
    {
        ...
        com.example.alldocument.library.fc.hwpf.sprm.SprmOperation sprmOperation = sprmBuffer.findSprm((short)0xd608);
        ...
        {
            com.example.alldocument.library.fc.hwpf.sprm.SprmOperation sprm = iterator.next();

            /*
             * TAPXs are actually PAPXs so we have to make sure we are only
             * trying to uncompress the right type of sprm.
             */
            if (sprm.getType() == com.example.alldocument.library.fc.hwpf.sprm.SprmOperation.TYPE_TAP
                || sprm.getType() == com.example.alldocument.library.fc.hwpf.sprm.SprmOperation.TYPE_PAP)
            {
                try
                {
