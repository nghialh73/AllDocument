...
final class SSTSerializer {
    ...
    public SSTSerializer( IntMapper<UnicodeString> strings, int numStrings, int numUniqueStrings )
    {
        ...
        int infoRecs = com.example.alldocument.library.fc.hssf.record.ExtSSTRecord.getNumberOfInfoRecsForStrings(strings.size());
    ...
    public void serialize(ContinuableRecordOutput out) {
        ...
        {
            if (k % com.example.alldocument.library.fc.hssf.record.ExtSSTRecord.DEFAULT_BUCKET_SIZE == 0)
            {
              int rOff = out.getTotalSize();
              int index = k/ com.example.alldocument.library.fc.hssf.record.ExtSSTRecord.DEFAULT_BUCKET_SIZE;
