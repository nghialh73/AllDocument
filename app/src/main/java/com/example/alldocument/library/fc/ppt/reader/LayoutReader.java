    ...
    private void processTextStyle(IControl control, PackagePart layoutPart, PGMaster pgMaster, PGLayout pgLayout, Element spTree)
    {
        for (Iterator< ? > it = spTree.elementIterator(); it.hasNext();)
        {
            ...
            if (txBody != null)
            {
                ...
                com.example.alldocument.library.fc.ppt.reader.StyleReader.instance().setStyleIndex(style);
                if (!PGPlaceholderUtil.instance().isBody(type))
                {
                    pgLayout.setStyleByType(type, com.example.alldocument.library.fc.ppt.reader.StyleReader.instance().getStyles(control, pgMaster, sp, lstStyle));
                }
                else if (idx > 0)
                {
                    pgLayout.setStyleByIdx(idx, com.example.alldocument.library.fc.ppt.reader.StyleReader.instance().getStyles(control, pgMaster, sp, lstStyle));
                }
