...
import com.example.alldocument.library.wp.view.BNView;
import com.example.alldocument.library.wp.view.LeafView;
import com.example.alldocument.library.wp.view.LineView;
...
public class LayoutKit
{
                          ...
                          ParagraphView para, long startOffset, int x, int y, int w, int h, int flag)
    {
        ...
        com.example.alldocument.library.wp.view.LineView line = (com.example.alldocument.library.wp.view.LineView) ViewFactory.createView(control, elem, elem, WPViewConstant.LINE_VIEW);
        ...
        com.example.alldocument.library.wp.view.BNView bnView = null;
        int bnViewWidth = -1;
        ...
        while (spanH > 0 && lineStart < elemEnd && breakType != WPViewConstant.BREAK_PAGE)
        {
            ...
            {
                line = (com.example.alldocument.library.wp.view.LineView)ViewFactory.createView(control, elem, elem, WPViewConstant.LINE_VIEW);
                line.setStartOffset(lineStart);
                para.appendChlidView(line);
            }
            keepOne = false;
     ...
     * @return
     */
    public int layoutLine(IControl control, IDocument doc, DocAttr docAttr, PageAttr pageAttr, ParaAttr paraAttr, com.example.alldocument.library.wp.view.LineView line,
                          com.example.alldocument.library.wp.view.BNView bnView, int x, int y, int w, int h, long maxEnd, int flag)
    {
        int breakType = WPViewConstant.BREAK_NO;
        int dx = 0;
        int dy = 0;
        ...
        com.example.alldocument.library.wp.view.LeafView leaf = null;
        IElement run;
        int lineWidth = 0;
        int lineHeigth = 0;
        ...
        while (spanW > 0 && pos < maxEnd || keepOne)
        {
            ...
            leaf = (com.example.alldocument.library.wp.view.LeafView)ViewFactory.createView(control, run, elem, WPViewConstant.LEAF_VIEW);
     ...
     * @return
     */
    private com.example.alldocument.library.wp.view.BNView createBNView(IControl control, IDocument doc, DocAttr docAttr, PageAttr pageAttr, ParaAttr paraAttr,
                                                          ParagraphView para, int x, int y, int w, int h, int flag)
    {
        if (paraAttr.listID >= 0 && paraAttr.listLevel >= 0
            || paraAttr.pgBulletID >= 0)
        {
            com.example.alldocument.library.wp.view.BNView bnView = (BNView)ViewFactory.createView(control, null, null, WPViewConstant.BN_VIEW);
