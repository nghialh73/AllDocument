...
import com.example.alldocument.library.fc.ddf.DefaultEscherRecordFactory;
...
import com.example.alldocument.library.fc.hssf.model.AbstractShape;
import com.example.alldocument.library.fc.hssf.model.CommentShape;
import com.example.alldocument.library.fc.hssf.model.ConvertAnchor;
import com.example.alldocument.library.fc.hssf.model.DrawingManager2;
import com.example.alldocument.library.fc.hssf.model.TextboxShape;
import com.example.alldocument.library.fc.hssf.record.AbstractEscherHolderRecord;
import com.example.alldocument.library.fc.hssf.record.CommonObjectDataSubRecord;
...
import com.example.alldocument.library.fc.hssf.record.EndSubRecord;
import com.example.alldocument.library.fc.hssf.record.GroupMarkerSubRecord;
...
import com.example.alldocument.library.fc.hssf.usermodel.HSSFChart;
...
import com.example.alldocument.library.fc.hssf.usermodel.HSSFShapeContainer;
import com.example.alldocument.library.fc.hssf.usermodel.HSSFShapeFactory;
import com.example.alldocument.library.fc.hssf.usermodel.HSSFShapeGroup;
import com.example.alldocument.library.fc.hssf.usermodel.HSSFTextbox;
...
public final class EscherAggregate extends AbstractEscherHolderRecord {
	...
	/** Maps shape container objects to their {@link TextObjectRecord} or {@link com.example.alldocument.library.fc.hssf.record.ObjRecord} */
	private Map<EscherRecord, com.example.alldocument.library.fc.hssf.record.Record> shapeToObj = new HashMap<EscherRecord, com.example.alldocument.library.fc.hssf.record.Record>();
	//chart records
	 private Map<EscherRecord, List<com.example.alldocument.library.fc.hssf.record.Record>> chartToObj = new HashMap<EscherRecord, List<com.example.alldocument.library.fc.hssf.record.Record>>();
	...
	{
	    int count = 0;
	    if((sid( records, currentDrawingRecord ) == com.example.alldocument.library.fc.hssf.record.DrawingRecord.sid || sid( records, currentDrawingRecord ) == com.example.alldocument.library.fc.hssf.record.ContinueRecord.sid)
	        && isObjectRecord( records, currentDrawingRecord + 1 ))
	    {
	        com.example.alldocument.library.fc.hssf.record.Record record = (com.example.alldocument.library.fc.hssf.record.Record) records.get( currentDrawingRecord + 1 );
	        count = 2;
	        //chart records
            if(record instanceof com.example.alldocument.library.fc.hssf.record.ObjRecord && ((com.example.alldocument.library.fc.hssf.record.ObjRecord)record).getSubRecords().get(0) instanceof CommonObjectDataSubRecord)
            {
                CommonObjectDataSubRecord commonObjectDataSubRecord = (CommonObjectDataSubRecord)((com.example.alldocument.library.fc.hssf.record.ObjRecord)record).getSubRecords().get(0);
                if(commonObjectDataSubRecord.getObjectType() == CommonObjectDataSubRecord.OBJECT_TYPE_CHART)
                {
                  //chart 
                    List<com.example.alldocument.library.fc.hssf.record.Record> chartRecordsList = new ArrayList<com.example.alldocument.library.fc.hssf.record.Record>();
                    currentDrawingRecord += 2;     
                    record = (com.example.alldocument.library.fc.hssf.record.Record) records.get( currentDrawingRecord );
                    while(record.getSid() != com.example.alldocument.library.fc.hssf.record.EOFRecord.sid)
                    {
    ...
    { // TODO - remove this method
        ...
        for (int i = currentIndex + 1; i < max; i++)
        {
            ...
            if (!(rb instanceof com.example.alldocument.library.fc.hssf.record.Record))
            {
                continue;
            }
            com.example.alldocument.library.fc.hssf.record.Record record = (com.example.alldocument.library.fc.hssf.record.Record)rb;
            if (record.getSid() == com.example.alldocument.library.fc.hssf.record.DrawingRecord.sid || record.getSid() == com.example.alldocument.library.fc.hssf.record.ContinueRecord.sid)
            {
                return i;
            }
        }
	...
	public static EscherAggregate createAggregate( List records, int locFirstDrawingRecord, DrawingManager2 drawingManager )
	{
		...
		while ( loc > -1 && loc + 1 < records.size()
				&& (sid( records, loc ) == com.example.alldocument.library.fc.hssf.record.DrawingRecord.sid || sid( records, loc ) == com.example.alldocument.library.fc.hssf.record.ContinueRecord.sid))
		{	
		    if(isObjectRecord( records, loc + 1 ))
		    {
		        if(sid( records, loc ) == com.example.alldocument.library.fc.hssf.record.ContinueRecord.sid )
		        {
		            dataSize += ((com.example.alldocument.library.fc.hssf.record.ContinueRecord)records.get(loc)).getDataSize();
		        }
		        else
		        {
		            dataSize += ((com.example.alldocument.library.fc.hssf.record.DrawingRecord)records.get(loc)).getData().length;
		        }
		    }			
				...
				&& (sid( records, loc ) == com.example.alldocument.library.fc.hssf.record.DrawingRecord.sid || sid( records, loc ) == com.example.alldocument.library.fc.hssf.record.ContinueRecord.sid))
		{
		    if(isObjectRecord( records, loc + 1 ))
            {
		        if(sid( records, loc ) == com.example.alldocument.library.fc.hssf.record.ContinueRecord.sid )
                {
		            com.example.alldocument.library.fc.hssf.record.ContinueRecord contd = (com.example.alldocument.library.fc.hssf.record.ContinueRecord) records.get( loc );
		            data = contd.getData();
                }
                else
                {
                    com.example.alldocument.library.fc.hssf.record.DrawingRecord drawingRecord = (com.example.alldocument.library.fc.hssf.record.DrawingRecord) records.get( loc );
		...
		loc = locFirstDrawingRecord;
		int shapeIndex = 0;
		agg.shapeToObj = new HashMap<EscherRecord, com.example.alldocument.library.fc.hssf.record.Record>();
		while ( loc > -1 && loc + 1 < records.size()
				&& (sid( records, loc ) == com.example.alldocument.library.fc.hssf.record.DrawingRecord.sid || sid( records, loc ) == ContinueRecord.sid))
		{
		    if(!isObjectRecord( records, loc + 1 ))
            {
		        loc = nextDrawingRecord(records, loc);
		    ...
		    com.example.alldocument.library.fc.hssf.record.Record record = (com.example.alldocument.library.fc.hssf.record.Record) records.get( loc + 1 );
		    try
		    {
		      //chart records
	            if(record instanceof com.example.alldocument.library.fc.hssf.record.ObjRecord && ((com.example.alldocument.library.fc.hssf.record.ObjRecord)record).getSubRecords().get(0) instanceof CommonObjectDataSubRecord)
	            {
	                ...
	                if(commonObjectDataSubRecord.getObjectType() == CommonObjectDataSubRecord.OBJECT_TYPE_CHART)
	                {
	                    ...
	                    List<com.example.alldocument.library.fc.hssf.record.Record> chartRecordsList = new ArrayList<com.example.alldocument.library.fc.hssf.record.Record>();
	                    loc += 2;     
	                    record = (com.example.alldocument.library.fc.hssf.record.Record) records.get( loc );
	...
	public int serialize( int offset, byte[] data )
	{
		...
		for ( int i = 1; i < shapes.size(); i++ )
		{
			...
			com.example.alldocument.library.fc.hssf.record.DrawingRecord drawing = new DrawingRecord();
			...
			com.example.alldocument.library.fc.hssf.record.Record obj = shapeToObj.get( shapes.get( i ) );
		...
		{
			com.example.alldocument.library.fc.hssf.record.Record rec = (com.example.alldocument.library.fc.hssf.record.Record)tailRec.get(i);
	...
	public int getRecordSize() {
		...
		{
			com.example.alldocument.library.fc.hssf.record.Record r = (com.example.alldocument.library.fc.hssf.record.Record) iterator.next();
	 ...
	 */
	Object associateShapeToObjRecord( EscherRecord r, com.example.alldocument.library.fc.hssf.record.ObjRecord objRecord )
	{
	...
	private void convertRecordsToUserModel(EscherContainerRecord shapeContainer, Object model) 
	{
		for(Iterator<EscherRecord> it = shapeContainer.getChildIterator(); it.hasNext();) 
		{
			...
			else if(r instanceof EscherTextboxRecord) 
			{
				...
				com.example.alldocument.library.fc.hssf.record.Record obj = shapeToObj.get(tbr);
			...
			else if(r instanceof EscherClientDataRecord && model instanceof HSSFChart) 
            {
                ...
                List<com.example.alldocument.library.fc.hssf.record.Record> recordsList = chartToObj.get(cdr);
...
//		lastShapeId = 1024;
	...
	{
		return sid( records, loc ) == com.example.alldocument.library.fc.hssf.record.ObjRecord.sid || sid( records, loc ) == TextObjectRecord.sid;
	}
...
//		clientAnchor.setDy2( (short) shape.getAnchor().getDy2() );
		...
		com.example.alldocument.library.fc.hssf.record.ObjRecord obj = new ObjRecord();
		...
		com.example.alldocument.library.fc.hssf.record.EndSubRecord end = new EndSubRecord();
