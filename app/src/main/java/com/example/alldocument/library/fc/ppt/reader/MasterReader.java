...
public class MasterReader
{
    ...
    private void processTextStyle(IControl control, PGMaster pgMaster, Element spTree) throws Exception
    {
        for (Iterator< ? > it = spTree.elementIterator(); it.hasNext();)
        {
            ...
            String type = com.example.alldocument.library.fc.ppt.reader.ReaderKit.instance().getPlaceholderType(sp);
            ...
            if (txBody != null)
            {
                ...
                com.example.alldocument.library.fc.ppt.reader.StyleReader.instance().setStyleIndex(styleIndex);
                if (!PGPlaceholderUtil.instance().isBody(type))
                {
                    pgMaster.addStyleByType(type, com.example.alldocument.library.fc.ppt.reader.StyleReader.instance().getStyles(control, pgMaster, sp, lstStyle));
                }
                else if (idx > 0)
                {
                    pgMaster.addStyleByIdx(idx, com.example.alldocument.library.fc.ppt.reader.StyleReader.instance().getStyles(control, pgMaster, sp, lstStyle));
                }
                
                styleIndex = com.example.alldocument.library.fc.ppt.reader.StyleReader.instance().getStyleIndex();
            }
        }
    }
    ...
    private void processStyle(IControl control, PGMaster pgMaster, Element master)
    {
        ...
        if (txStyles != null)
        {
            ...
            pgMaster.setTitleStyle(com.example.alldocument.library.fc.ppt.reader.StyleReader.instance().getStyles(control, pgMaster, null, style));
            
            style = txStyles.element("bodyStyle");
            pgMaster.setBodyStyle(com.example.alldocument.library.fc.ppt.reader.StyleReader.instance().getStyles(control, pgMaster, null, style));
            
            style = txStyles.element("otherStyle");
            pgMaster.setDefaultStyle(com.example.alldocument.library.fc.ppt.reader.StyleReader.instance().getStyles(control, pgMaster, null, style));
