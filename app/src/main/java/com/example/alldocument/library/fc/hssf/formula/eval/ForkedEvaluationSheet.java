 ...
 * the master cell object is logically replaced with a {@link com.example.alldocument.library.fc.hssf.formula.eval.ForkedEvaluationCell} instance, which
...
final class ForkedEvaluationSheet implements EvaluationSheet {
	 ...
	 */
	private final Map<RowColKey, com.example.alldocument.library.fc.hssf.formula.eval.ForkedEvaluationCell> _sharedCellsByRowCol;

	public ForkedEvaluationSheet(EvaluationSheet masterSheet) {
		_masterSheet = masterSheet;
		_sharedCellsByRowCol = new HashMap<RowColKey, com.example.alldocument.library.fc.hssf.formula.eval.ForkedEvaluationCell>();
	}

	public EvaluationCell getCell(int rowIndex, int columnIndex) {
		RowColKey key = new RowColKey(rowIndex, columnIndex);

		com.example.alldocument.library.fc.hssf.formula.eval.ForkedEvaluationCell result = _sharedCellsByRowCol.get(key);
		if (result == null) {
	...
	public com.example.alldocument.library.fc.hssf.formula.eval.ForkedEvaluationCell getOrCreateUpdatableCell(int rowIndex, int columnIndex) {
		RowColKey key = new RowColKey(rowIndex, columnIndex);
		...
		if (result == null) {
			...
			result = new com.example.alldocument.library.fc.hssf.formula.eval.ForkedEvaluationCell(this, mcell);
